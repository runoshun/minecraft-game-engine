...
The current runtime exposes only `world.setBlock`. Each call performs a normal server block update and may synchronously obtain the target chunk, so thousands of writes in one script tick would violate the runtime's tick-budget goals.
...
Until the runtime gains a bounded batched/fill world-edit capability, the example must amortize projection across ticks:

- queue the required block writes after dungeon generation
- execute only a bounded number of writes per server tick
...
- a future `world.batch`/`world.fill` capability can replace the projection adapter without changing the dungeon generator or turn rules
