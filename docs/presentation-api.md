...
That local dev environment needed extra heap for the embedded TypeScript compiler and a temporarily relaxed watchdog to get through cold GraalJS script initialization; the production source was restored to the normal 100 ms hard budget afterwards. `ui.panel`, virtual-container click delivery, and Dialog click delivery still require player-driven E2E validation on the main server after the 0.2.0 JAR is deployed.
