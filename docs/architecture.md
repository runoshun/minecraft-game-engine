...
## Proposed Portable v27 compound conditions

ADR 0040 proposes a bounded recursive condition tree for Portable v27. V1-v26 conditions remain single scalar comparisons; v27 would additionally permit `all`, `any`, and `not` condition nodes with explicit depth/node/child bounds. The same v27 condition type is intended for ordinary action `if`, reduction predicates, and existing declarative `when:` surfaces so the engine retains one pure condition language rather than separate action and presentation boolean systems.

The authoring proposal is explicit opt-in through `game.condition.all(...)`, `game.condition.any(...)`, and `game.condition.not(...)`. Existing ADR 0038 `whenAll` / `whenAny` / `unless` / `choose` / `match` remain build-time compatibility sugar and therefore do not silently raise older source to v27. This preserves previous version inference while giving authors a non-duplicating condition representation for large guarded bodies and reusable declarative conditions.

Vanilla lowering would evaluate a compound tree through bounded generated condition functions returning 0/1, materialize that result in compiler-private scratch, and dispatch from it once. Comparison-only conditions retain their current direct scoreboard fast path. The proposal removes IR action multiplication caused by expanding boolean OR/AND guards; it does not increase the 2,048-action ceiling.

Reusable runtime rules/procedures are explicitly deferred from v27. Othello showed that repeated TypeScript helper calls can also duplicate large action trees, but a runtime call facility requires separate decisions for parameters/locals, lexical player/session/placeable specialization, recursion/cycle rejection, action accounting, and execution-context preservation. ADR 0040 treats that as a possible follow-up rather than coupling it to compound conditions.
