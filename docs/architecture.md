...
- no runtime generic arrays/collections, arbitrary packet-event dispatch, arbitrary inventory/form/dialog API, arbitrary NBT/storage API, or arbitrary Minecraft queries; v21 provides bounded static rich native-dialog content, v22 provides bounded static mannequin profile/pose/equipment presentation, v23 provides bounded right-click/use events through compiler-owned interaction entities, and v24 provides active-instance interaction-controller binding; free-form text input, generic multi-field/dynamic forms, arbitrary click/attack events or commands, inventory GUI ownership, generic entity mutation, persistent controller identity across reload, generic player identity values, and runtime entity collections remain unsupported;
