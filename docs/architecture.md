...
Portable multiplayer is **planned but not implemented** as v12; ADR 0020 records the accepted design direction. The core model separates `PlayerSet`, `PlayerContext`, shared state, and player-local state rather than adding a name/UUID-oriented `game.player(...)` lookup. The intended first milestone is one shared game instance with N participants: `game.players()` yields the participant set, `game.forEachPlayer(players, player => ...)` enters a deterministic player execution context, `player.state(...)` and `player.input.*` are player-local, camera/HUD declarations target player sets or the current player, disconnect preserves player-local state for reconnect, and namespace cleanup removes all generated player-local scoreboard state including offline entries. Multiple simultaneous game sessions, arbitrary player-private world projections, and independent per-player vanilla sidebars are not part of the initial v12 scope.
...
ADR 0020 defines the accepted multiplayer direction; this section is design intent, **not current v11 behavior**. The initial target is one shared portable game instance with multiple participants, not multiple concurrent match/session instances.

- `PlayerSet` is the first-class audience/participant abstraction. The default `game.players()` is planned to mean the current game's online participant set; the first implementation may use all online players as the membership policy while keeping membership abstract in the IR so later lobby/team/session policies do not require name- or UUID-fixed APIs.
- `PlayerContext` is entered explicitly through a construct such as `game.forEachPlayer(players, player => ...)`. This is a compiler primitive lowered to `execute as <participants>`, not an unrestricted JavaScript runtime loop. The exact surface spelling may still change before implementation, but the separation between set selection and player execution context is part of the accepted design.
- `game.state(...)` remains shared game state. `player.state(...)` is planned player-local state keyed by the Minecraft scoreboard holder for that player. Player-local state survives disconnect/reconnect during an active game; namespace cleanup removes the owning objectives so offline entries are cleaned without selecting offline entities.
- Existing `first_player_*` input bindings are a legacy single-controller representation. Multiplayer input is planned as `player.input.*` evaluated in player context, with held input and edge state stored per player.
- Shared state mutation from inside an unordered player iteration is not automatically allowed. The v12 IR must either reject order-sensitive shared writes from player context or introduce explicit deterministic aggregation primitives; ordinary per-player state mutation remains valid.
- Shared fixed cameras may target a `PlayerSet`. One camera carrier can be observed by multiple spectator clients. Player names/UUIDs are not embedded in generated packs. Per-player cameras with distinct positions/carriers are a later extension.
- Per-player actionbar HUD is in the initial target because vanilla commands can address the current player. The existing vanilla scoreboard sidebar remains shared/global; independent per-player portable sidebars are not promised by v12 because the vanilla display slot is server-global.
- Sound/effect audiences should be participant-scoped where vanilla supports it so unrelated online players do not receive game presentation. Shared actors, block/text displays, and world projection remain server-global scene state unless a later primitive explicitly introduces player-private presentation.
- Join initializes missing player-local state; disconnect does not erase it; reconnect reuses it. Cleanup/replacement owns generated objectives/entities/force-loads but still does not own or persist player gamemode. Spectator entry/exit remains external session lifecycle unless a future offline-safe ownership mechanism is designed.
- Fabric compatibility adapters must implement the same semantics over their already per-player input/UI/camera facilities; they must stop collapsing portable behavior to `input.players()[0]`.

The implementation is expected to require a new portable IR version because player-scoped values and execution contexts cannot be represented by the current global fake-score holders without introducing order-dependent writes.
