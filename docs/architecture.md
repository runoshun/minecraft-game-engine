...
## Proposed bounded item/placeable object v25

ADR 0036 is the proposed next capability for item-backed, runtime-placeable world objects. It does **not** claim that datapacks can register a new native Minecraft block/item type. The portable model is a bounded static `ItemTemplate` plus a compile-time-sized `PlaceableType` slot pool. A placeable item uses a compiler-controlled vanilla placement carrier, and each accepted placement becomes one active slot with compiler-owned anchor position/cardinal orientation, instance-local state, child presentation, and child interactions/controllers.

The motivating lowering is proved at the vanilla boundary: Minecraft 26.1 can place an `minecraft:armor_stand` item whose compiler-authored `minecraft:entity_data` produces an invisible Marker carrying a stable tag while preserving the actual placement `Pos` and player-facing `Rotation`. The proposed allocator consumes that Marker into the lowest free slot inside the existing ownership rectangle. The Marker is a transport only; game code receives no raw entity/NBT/selector/raycast access and no exact `onPlace(player)` identity.

The proposed instance context owns local coordinates. Presentation lowering performs `instance anchor + local value` and a cardinal local-to-world transform internally, so a pinball can keep ball/flipper collision and physics in table-local 2D coordinates while Displays/interactions follow the placed cabinet. The proposal also adds a bounded item-display child that renders only compiler-known ItemTemplates. Custom namespaced item models remain ordinary external resource-pack assets; a custom resource pack changes appearance but does not require a client mod.

V25 is intentionally planned as active-instance state: removal/pickup frees and resets a slot, `/reload`/replacement clears every placed instance and pending placement Marker, and `portable/cleanup` removes all generated placeable resources. Persistence of placed furniture across reload is explicitly separate because it requires unloaded-chunk and schema/migration semantics. Native block collision, mining, redstone, fluids, pistons and block-state behavior are also outside the proposed placeable contract.

The proposed bounds are 32 item templates, 8 placeable types, 1..16 slots per type, 32 aggregate placeable slots, 16 state fields per template / 256 expanded instance-state cells, and 256 expanded child presentation entities. Child interactions remain inside the existing 64 interaction/controller bank and the existing 2,048-action expanded-program limit still applies. These values remain proposed until v25 implementation/acceptance.
...
After those priorities, ADR 0036 proposes bounded item templates and active-instance placeable object slots as the next concrete expression capability, motivated by an item-placeable pinball cabinet. Until implemented, it remains a planned design rather than current Portable behavior. Additional capability gaps include 3D/swept collision, deliberately scoped Minecraft world/entity queries, pathfinding/topology helpers, and generic runtime collections where existing Grid/fixed-slot patterns prove insufficient.
...
- no runtime generic arrays/collections, arbitrary packet-event dispatch, arbitrary inventory/form/dialog API, arbitrary NBT/storage API, or arbitrary Minecraft queries; v21 provides bounded static rich native-dialog content, v22 provides bounded static mannequin profile/pose/equipment presentation, v23 provides bounded right-click/use events through compiler-owned interaction entities, and v24 provides active-instance interaction-controller binding; ADR 0036 proposes but does not yet implement bounded item templates/placeable slots; free-form text input, generic multi-field/dynamic forms, arbitrary click/attack events or commands, inventory GUI ownership, generic entity mutation, persistent controller identity across reload, persistent placed objects, native custom block semantics, generic player identity values, and runtime entity collections remain unsupported;
